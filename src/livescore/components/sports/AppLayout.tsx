...
const AppLayout: React.FC = () => {
  ...
  const { matches: baseMatches, source, loading, error, isFetching } = useScoreSimulator() as { matches: LiveMatch[]; source: string; loading: boolean; error: string | null; isFetching: boolean; refetch: () => void };
  ...
  const jsonLd = useMemo(() => {
    const arr: Record<string, unknown>[] = [getOrganizationJsonLd(), getWebsiteJsonLd(), getBreadcrumbJsonLd(breadcrumbItems)];
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] text-white">
      <SEO pageKey={pageKey} jsonLd={jsonLd} />
