...
const API_HOST = (process.env.NEXT_PUBLIC_ALLSPORTS_API_HOST ?? process.env.VITE_ALLSPORTS_API_HOST) || 'allsportsapi2.p.rapidapi.com';
const API_KEY = process.env.NEXT_PUBLIC_RAPIDAPI_KEY ?? process.env.VITE_RAPIDAPI_KEY;
...
export async function getAllSportsLiveMatches(): Promise<{
  ...
  if (!API_KEY || (process.env.NEXT_PUBLIC_ENABLE_ALLSPORTS_API ?? process.env.VITE_ENABLE_ALLSPORTS_API) !== 'true') {
